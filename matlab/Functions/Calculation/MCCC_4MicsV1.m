...
function [ det_Ra_p_theta ] = MCCC_4MicsV1( y_t, N_SIGBLOCK, THETA, PHI, fa, A, c )
...
det_Ra_p_theta = zeros(1,length(THETA.MIN:THETA.RESOLUTION:THETA.MAX));
...
%for p_phi = p_phi_min:p_phi_max
    for theta = THETA.MIN:THETA.RESOLUTION:THETA.MAX
     ...
     D=     [sigma_y(1,1)          r_xy(1,Offset+L(1))   r_xy(2,Offset+L(2))  r_xy(3,Offset+L(3)); ...
             r_xy(1,Offset+L(1))   sigma_y(2,1)          r_xy(4,Offset+L(4))  r_xy(5,Offset+L(5)); ...    
             r_xy(2,Offset+L(2))   r_xy(4,Offset+L(4))   sigma_y(3,1)         r_xy(6,Offset+L(6)); ...
             r_xy(3,Offset+L(3))   r_xy(5,Offset+L(5))   r_xy(6,Offset+L(6))  sigma_y(4,1) 
            ];
        %);
...
% 
 figure(667);
 subplot(2,1,1)
 stem(THETA.MIN:THETA.RESOLUTION:THETA.MAX,det_Ra_p_theta);
 grid on
 xlabel('DOA-Theta [°]')
