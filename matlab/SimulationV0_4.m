...
AArray.AngleRes = round( rad2deg( asin(c/(Array.MinDistance * fa)) ) );
...
for i=1:EDMA.NumOfSim
    ...
    if( Buf.Phi_Est(i) == INDEX_INVALID )
        plot(i, Buf.Phi_Est(i),'x','Color','m');
    else
        plot(i, Buf.Phi_Est(i),'rs','Color','b');
    end
end
...
for i=1:EDMA.NumOfSim
    ...
    if( Buf.Theta_Est(i) == INDEX_INVALID )
        plot(i, Buf.Theta_Est(i),'x','Color','m');
    else
        plot(i, Buf.Theta_Est(i),'rs','Color','b');
    end
end
...
for i=1:EDMA.NumOfSim
    ...
    if( Buf.Phi_Hist_Est_Id(i) == INDEX_INVALID )
        plot(i, Buf.Phi_Hist_Est_Id(i),'x','Color','m');
    else
        plot(i, Phi.LUT(Buf.Phi_Hist_Est_Id(i)),'rs','Color','b');
    end
end
plot(1:EDMA.NumOfSim, rad2deg(EDMA.Phi),'r');
grid;
...
for i=1:EDMA.NumOfSim
    ...
    if( Buf.Theta_Hist_Est_Id(i) == INDEX_INVALID )
        plot(i, Buf.Theta_Hist_Est_Id(i),'x','Color','m');
    else
        plot(i, Theta.LUT(Buf.Theta_Hist_Est_Id(i)),'rs','Color','b');
    end
end
plot(1:EDMA.NumOfSim, rad2deg(EDMA.Theta),'r');
grid;
